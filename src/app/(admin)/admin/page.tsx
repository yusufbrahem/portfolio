...
export default async function AdminDashboard() {
  ...
  const [skillsCount, projectsCount, experienceCount, aboutContent, architectureContent, personInfo, heroContent, portfolio, portfolioVisibility] = await Promise.all([
    prisma.skill.count({ where: { skillGroup: whereClause } }),
    ...
    portfolioId
      ? prisma.portfolio.findUnique({ where: { id: portfolioId }, select: { status: true, rejectionReason: true } })
      : session.user.portfolioId
      ? prisma.portfolio.findUnique({ where: { id: session.user.portfolioId }, select: { status: true, rejectionReason: true } })
      : null,
    getPortfolioVisibility(),
  ]);

  const principlesCount = aboutContent ? await prisma.aboutPrinciple.count({ where: { aboutContentId: aboutContent.id } }) : 0;
  const pillarsCount = architectureContent ? await prisma.architecturePillar.count({ where: { architectureContentId: architectureContent.id } }) : 0;
    ...
    <Container>
      <div className="space-y-6">
        ...
        {personInfo && (
          <div className="border border-border bg-panel rounded-lg p-6">
            ...
            <div className="flex items-center gap-6">
              {(personInfo as { avatarUrl?: string | null })?.avatarUrl ? (
                <div className="h-24 w-24 rounded-full overflow-hidden border-2 border-border bg-panel2 flex-shrink-0">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={(personInfo as { avatarUrl: string }).avatarUrl} alt={`${personInfo.name} avatar`} className="h-full w-full object-cover" />
                </div>
              ) : (
              ...
              <div className="flex-1">
                ...
                <Link href="/admin/account" className="mt-3 inline-block text-sm text-accent hover:underline">
                  Edit profile →
                </Link>
              </div>
            </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          <Link href="/admin/skills" className="border border-border bg-panel rounded-lg p-6 hover:bg-panel2 transition-colors">
          ...
          <Link href="/admin/projects" className="border border-border bg-panel rounded-lg p-6 hover:bg-panel2 transition-colors">
            <div className="flex items-center gap-3 mb-2">
          ...
          <Link href="/admin/experience" className="border border-border bg-panel rounded-lg p-6 hover:bg-panel2 transition-colors">
            <div className="flex items-center gap-3 mb-2">
          ...
          <Link href="/admin/about" className="border border-border bg-panel rounded-lg p-6 hover:bg-panel2 transition-colors">
            <div className="flex items-center gap-3 mb-2">
              <User className="h-5 w-5 text-accent" />
              <h2 className="text-lg font-semibold text-foreground">About</h2>
            </div>
            <p className="text-sm text-muted">{aboutContent ? `${principlesCount} principles` : "Not configured"}</p>
          </Link>
          <Link href="/admin/architecture" className="border border-border bg-panel rounded-lg p-6 hover:bg-panel2 transition-colors">
            <div className="flex items-center gap-3 mb-2">
              <Building2 className="h-5 w-5 text-accent" />
              <h2 className="text-lg font-semibold text-foreground">Architecture</h2>
            </div>
            <p className="text-sm text-muted">{architectureContent ? `${pillarsCount} pillars` : "Not configured"}</p>
          </Link>
          <Link href="/admin/contact" className="border border-border bg-panel rounded-lg p-6 hover:bg-panel2 transition-colors">
            <div className="flex items-center gap-3 mb-2">
              <Mail className="h-5 w-5 text-accent" />
              <h2 className="text-lg font-semibold text-foreground">Contact</h2>
            </div>
            <p className="text-sm text-muted">{personInfo ? "Configured" : "Not configured"}</p>
          </Link>
        </div>
          ...
          {session.user.role === "super_admin" && !portfolioId ? (
            <div className="border border-border bg-panel rounded-lg p-6">
              ...
              <p className="mt-2 text-sm text-muted">
                To edit hero content as a super admin, impersonate a portfolio first (read-only mode prevents writes). Stop impersonation to write, then switch to the target user to edit their hero.
              </p>
            </div>
          ) : (
            <HeroManager
